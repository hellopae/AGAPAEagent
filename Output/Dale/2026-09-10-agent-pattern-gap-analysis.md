...
## ⚠️ บันทึกแก้ไขหลังการทดสอบ — เพิ่มโดย Claudy 10 ก.ย. 2569 (หลัง QA ผ่านแล้ว)

**ข้อเสนอ #1 ของรายงานนี้ (ซ่อม hook chain) ถูกยกเลิก — สมมุติฐานที่รายงานตั้งไว้ผิด**

รายงานฉบับนี้สรุปว่า "hook ตัวแรกของแต่ละ event block ทำงาน ตัวที่ 2/3 ไม่ทำงาน" และตั้งสมมุติฐานว่า
hook หลายตัวแย่ง stdin เดียวกัน (รายงานติดป้ายไว้ถูกแล้วว่าเป็น **สมมุติฐาน ยังต้องยืนยัน** — Chris ตรวจข้อนี้ผ่าน)

Claudy ทดสอบตามที่รายงานเสนอ (ใส่ตัวบันทึกก่อนรื้อ) แล้วได้ผลตรงข้าม:

- ใส่ `scripts/hook-log.mjs` เข้าทั้ง 4 hook — บันทึก **จำนวนไบต์ที่อ่านได้จาก stdin** ต่อการยิงหนึ่งครั้ง
- รัน `claude -p` จากโฟลเดอร์โปรเจกต์ 2 ครั้ง (ครั้งที่ 2 delegate `libby-index` จริงเพื่อให้ครบทั้ง 3 block)
- ผล: **hook ยิงครบทุกตัว และทุกตัวอ่าน stdin ได้ครบเท่ากัน**
  `PreToolUse` → gate 660 ไบต์ / status 660 ไบต์ · `SubagentStop` → gate 738 / status 738 · `Stop` → gate + skill ได้เท่ากันทั้ง 2 รอบ
- **`.gate-state.json` และ `.skill-state.json` ถูกเขียนใหม่จริงทั้งคู่** และ hook-skill เด้งถามเรื่องเก็บ skill ตามที่ออกแบบไว้

**สาเหตุจริงของหลักฐานที่รายงานพบ (mtime ค้าง 29 ก.ค. / ไม่มี `.skill-state.json`)**
บนแมคเครื่องนี้ **ไม่มี session ที่เปิดจากโฟลเดอร์โปรเจกต์เลยตั้งแต่ 29 ก.ค.** —
`~/.claude/projects/-Users-agapae-Documents-Work-PAE-Claude-AGAPAE-Agent/` มี transcript แค่ 2 ไฟล์ (เทสต์ของวันนี้)
ขณะที่โฟลเดอร์ session ที่เปิดจาก home มี 22 ไฟล์เฉพาะเดือน ก.ย.
→ เปิดจาก home = ไม่โหลด `.claude/` = **ไม่มี hook ยิงเลยแม้แต่ตัวแรก** ซึ่งคือ P0 ที่ปิดไปแล้ว 1 ส.ค. ไม่ใช่บั๊กตัวใหม่

**ส่วนที่รายงานยังถูก:** ผลกระทบที่บรรยายไว้เป็นความจริง — ในช่วง 6 สัปดาห์ที่ทำงานจาก home
fact-check gate กับ DoD gate **ไม่ได้บังคับอะไรจริง** และข้อความใน `CLAUDE.md`/`HANDOFF.md` ก็ไม่ตรงกับความจริงในช่วงนั้น
รวมทั้งข้อสังเกตว่า **ไม่มี regression test ของ agent เลย** ซึ่งเป็นเหตุให้เรื่องนี้ไม่มีใครจับได้ — ข้อนั้นยังยืนอยู่

**ทางแก้ที่ใช้จริง:** เพิ่มฟังก์ชัน `agapae` ใน `~/.zshrc` (cd เข้าโฟลเดอร์แล้วเปิด claude ให้เอง) — แก้ที่ความเคยชิน ไม่ใช่ที่โค้ด
ตัวบันทึกคงไว้ในระบบเพื่อใช้วินิจฉัยครั้งหน้า: `node scripts/hook-log-report.mjs`
