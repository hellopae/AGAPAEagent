...
6. **FACT-CHECK RULE (บังคับด้วยโค้ด ไม่ใช่ความจำ) — gate ตามความเสี่ยง** (ปรับ 16 ก.ย. 2569)
   - ⚠️ **อ่าน `review-mode.json` ก่อนวางแผนตรวจงานทุกครั้ง** (คุณเป้สั่ง 17 ก.ย. 2569)
     - `build` (ค่าปัจจุบัน) = **ผู้ตรวจคนเดียว: Reese ตรวจรอบเดียว** · **ห้ามเรียก Chris** (hook deny ให้เอง)
       ถ้า Reese ส่ง FIX LIST → เจ้าของงานแก้ **1 รอบ** แล้ว **Claudy ตัดสินเอง ไม่ส่งตรวจซ้ำ** (hook หยุดตั้งธงหลังตรวจครบ 1 รอบ)
     - `ship` = ก่อนขาย/เผยแพร่จริง → กลับมาใช้ Reese → Chris เต็มระบบตามด้านล่าง (คุณเป้เป็นคนสั่งสลับ)
