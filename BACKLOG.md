...
- [x] **🔴 P0 (เปิด-ปิดวันเดียว 10 ก.ย. 2569): "hook ตัวที่ 2/3 ไม่ยิง" — ✅ พิสูจน์แล้วว่า ไม่ใช่บั๊ก**
  Dale รายงานว่า `.gate-state.json` ค้าง 29 ก.ค. และ `.skill-state.json` ไม่มีไฟล์เลย แล้วตั้งสมมุติฐานว่า
  hook หลายตัวใน block เดียวแย่ง stdin กัน → ตัวหลังได้ของว่าง
  **Claudy ทดสอบจริงแล้ว สมมุติฐานนี้ผิด** (10 ก.ย. 2569 — ใช้เวลา ~25 นาที)
  - ใส่ตัวบันทึก `scripts/hook-log.mjs` เข้าทั้ง 4 hook (บันทึก hook/mode/**จำนวนไบต์ที่อ่านจาก stdin**/pid/session)
    อ่านผลด้วย `node scripts/hook-log-report.mjs`
  - รัน `claude -p` จากโฟลเดอร์โปรเจกต์ 2 ครั้ง (ครั้งที่ 2 delegate `libby-index` จริง) ผล:
    **hook ยิงครบทุกตัวทุก block และทุกตัวอ่าน stdin ได้ครบเท่ากัน** —
    `PreToolUse` gate 660 ไบต์ + status 660 ไบต์ · `SubagentStop` gate 738 + status 738 · `Stop` gate + skill 613/869
  - **`.gate-state.json` และ `.skill-state.json` ถูกเขียนใหม่จริงทั้งคู่** และ hook-skill เด้งถามเรื่องเก็บ skill ตามที่ออกแบบ
  - เหตุที่ state ค้าง 29 ก.ค.: **บนแมคเครื่องนี้ไม่มี session ที่เปิดจากโฟลเดอร์โปรเจกต์เลยตั้งแต่ 29 ก.ค.**
    (`~/.claude/projects/-Users-agapae-Documents-Work-PAE-Claude-AGAPAE-Agent/` มี transcript แค่ 2 ไฟล์ = เทสต์ของวันนี้
    ขณะที่โฟลเดอร์ของ session ที่เปิดจาก home มี 22 ไฟล์เฉพาะเดือน ก.ย.)
    → **นี่คืออาการเดียวกับ P0 ที่ปิดไป 1 ส.ค. ไม่ใช่บั๊กตัวใหม่** — เปิดจาก home = ไม่โหลด `.claude/` = ไม่มี hook เลย
    _(commit รูปแบบ `worklog: <Agent> — …` ที่มีในวันที่ 8 และ 10 ก.ย. แสดงว่า hook ทำงานอยู่ที่อื่นที่ไม่ใช่แมคเครื่องนี้
    น่าจะเป็น session จากเว็บ/cloud ซึ่งมีไฟล์ระบบของตัวเอง — ข้อนี้เป็นสมมุติฐาน ยังไม่ยืนยัน)_
  - **สรุปที่ต้องจำ:** ผลของ Dale ถูกในแง่ "gate ไม่ได้บังคับอะไรจริงในช่วง 6 สัปดาห์ที่ทำงานจาก home"
    แต่ **สาเหตุที่เขาเดาผิด** และ **ไม่มีอะไรต้องแก้ในโค้ด** — ห้ามเสียเวลารื้อ `hook-chain.mjs` ตามข้อเสนอเดิม
  - ✅ ทางแก้ที่ทำแล้ว: เพิ่มฟังก์ชัน `agapae` ใน `~/.zshrc` — พิมพ์ `agapae` แล้วมันจะ `cd` เข้าโฟลเดอร์
    แล้วเปิด `claude` ให้เอง (แก้ที่ต้นเหตุคือความเคยชิน ไม่ใช่ที่โค้ด hook)
  - ตัวบันทึกยังอยู่ในระบบ ไม่มีต้นทุน (append บรรทัดเดียว, `.hook-log.jsonl` gitignore)
    ถ้าสงสัยว่า hook ไม่ทำงานอีก ให้รัน `node scripts/hook-log-report.mjs` ก่อนตั้งสมมุติฐานใด ๆ

- [x] **🔴 hook ไม่ยิงเมื่อเปิด session จากนอกโฟลเดอร์โปรเจกต์** ✅ ปิด 1 ส.ค. 2569
  **สรุปที่ Kittanate ตัดสิน:** เปิด Claude Code จากโฟลเดอร์นี้ตรง ๆ แล้วไม่มีปัญหา —
  ไม่ต้องแก้ให้ hook ทำงานข้าม cwd · **กติกาถาวร: เปิด session จาก `AGAPAE Agent/` เสมอ**
  (ถ้าวันไหนเปิดจากที่อื่นแล้วเจออาการเดิม = เปิดใหม่จากโฟลเดอร์นี้ ไม่ต้องไล่ดีบัก)
  _หมายเหตุตามจริง: ปิดด้วยการยืนยันจาก Kittanate ไม่ได้รันงานทดสอบ delegate ใหม่ในวันปิด_
  _(รายละเอียดอาการเดิมเก็บไว้ด้านล่างเป็นบันทึก)_
...
- [x] ~~**แท็บ Meeting — บันทึกการประชุมบน dashboard**~~ ❌ **ยกเลิกทั้งชุด 8 ส.ค. 2569 (Kittanate สั่ง)**
  เหตุผล: ไม่เอาโหมดที่ให้ agent ถกเถียงกัน — กลับไปใช้แบบเดิมคือ **Claudy มอบหมายงานตามความเหมาะสม แล้วสรุปผลงาน**
  ถอดออกแล้ว: `scripts/build-meeting.mjs` · `meetings.json` · แท็บ Meeting + การ์ด Meeting ใน Widgets (`index.html`)
  · ชิป "⚖ ประชุม" ใน Work Log · หัวข้อใน `scripts/README.md`
  เหลือไว้: เอกสารงานเดิมใน `Output/` (เป็นผลงานจริงของทีม) · โค้ดเก่าอยู่ใน git history
  ค้าง: doc Firestore `agents/meetings` ไม่มีอะไรอ่านแล้ว ลบทิ้งได้เมื่อสะดวก
  _(บันทึกเดิมตอนสร้างเก็บไว้ด้านล่าง)_
  ~~✅ 31 ก.ค. 2569 (Kittanate สั่ง 31 ก.ค.)~~
...
- [x] **ทดสอบ agent ใหม่ 3 ตัว (Libby / Mind / Dale)** ✅ ปิด 1 ส.ค. 2569 —
  ทั้ง 3 ตัวมีผลงานจริงผ่าน pipeline แล้ว (Mind ผ่าน QA · Dale build แท็บ Meeting · Libby บันทึกใบแรก 31 ก.ค.)
  และเงื่อนไข hook ปิดไปพร้อมข้อบนแล้ว → ไม่ต้องรันงานทดสอบแยกอีก
  _(บันทึกการวินิจฉัยเดิมเก็บไว้ด้านล่าง)_
  ~~เดิม: delegate งานเล็ก 1 ชิ้นต่อตัวผ่าน Task tool~~
...
- [ ] 🛑 **[พัก — Kittanate สั่งยกเลิกไปก่อน 1 ส.ค. 2569]** **เกียรติบัตร Express — 2 ปุ่มอัปโหลดใหม่**
  ยังไม่เริ่ม pipeline · งานเตรียมทั้งหมด (สำรวจโค้ด + artwork 303×216mm + ผลวัด slot) ทำไว้ครบแล้ว
  พร้อมหยิบต่อได้ทันทีเมื่อ Kittanate สั่งรื้อฟื้น — **ห้ามเริ่มเองจนกว่าจะได้ไฟเขียว**
  _(รายละเอียดโจทย์เดิมทั้งหมดอยู่ด้านล่าง — เก็บไว้ไม่ตัด)_

  ~~เดิม (สั่ง 16 ก.ค. 2569)~~ — repo:
...
## 📣 Addy (Marketing & Demand Gen)

_สร้าง 21 ส.ค. 2569 — ดู `Output/Addy/2026-08-21-addy-charter.md` สำหรับที่มาทั้งหมด_

- [ ] **P0: baseline จากใบสั่งงาน — งานแรกก่อนแตะแคมเปญใดๆ**
  เครื่องว่างเพราะฤดูกาล / หมวดหด / หรืองานเล็กลง — ทั้ง Reese และ Nick ยืนยันตรงกันว่า
  ถ้าข้ามขั้นนี้จะได้ agent ที่ดูขยันแต่พิสูจน์ไม่ได้ว่าช่วยหรือไม่ช่วย
  ⛔ ทุกข้อข้างล่างเรียงลำดับใหม่ทันทีที่รู้คำตอบ
- [ ] **P0: ชีตนับมือ M2 (คำขอราคาต่อสัปดาห์ แยกช่องทาง) + M3 (อัตราปิดใบเสนอราคา)**
  วัดได้พรุ่งนี้ ไม่ต้องต่อระบบ วันละ 1 นาที · 4 สัปดาห์แรกคือ baseline ห้ามตัดสินก่อนหน้านั้น
- [ ] **P1: เช็คช่องทางจัดซื้อภาครัฐ** — งานพิมพ์นับเป็นพัสดุในโควตา 30% ของ e-GP หรือไม่
  และ TANAPAT ขึ้นทะเบียน Thai SME-GP แล้วหรือยัง (ต้องการ Kittanate ตอบ)
  ⛔ ห้ามลงแรงช่องทางนี้ก่อนได้คำตอบทั้งสองข้อ
- [ ] P1: Prospect list 30–50 ราย ในหมวดที่กระดาษยังเป็นตัวสินค้า พร้อมคอลัมน์ "โทรแล้ว/ยัง"
- [ ] P1: Capability brief รายหมวด (กล่อง/ฉลาก/องค์กร/หนังสือ) — **ห้ามใส่ตัวเลขต้นทุน**
- [ ] P2: Google Business Profile checklist + keyword map
- [ ] P2: ระบบตาม quote D+3/D+10 + บันทึกเหตุผลที่แพ้ → ส่ง log ดิบให้ Nick
- [ ] เสมอ: เปิดช่องทางใหม่ได้ไตรมาสละ 1 ช่อง มีวันตัดสินและเกณฑ์เขียนไว้ล่วงหน้า
- [ ] เสมอ: ทุก deliverable จบที่ "สิ่งที่ต้องลงมือทำต่อ" — ห้ามส่ง "แผน" เป็นของชิ้นสุดท้าย
