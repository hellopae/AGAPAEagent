...
Claude cloud routine ทั้ง 6 ตัวคงเวลาเดิม Codex เป็น fallback หลัง Claude; ถ้า Codex ชน limit Claude ยังรันก่อนในรอบปกติถัดไป ไม่ใช่การเรียก Claude ซ้ำทันทีในวันเดียวกัน
...
เตรียม Gmail/Todoist connector ใน Codex ก่อนให้ routine ที่เกี่ยวข้องทำงาน
...
หลังตั้ง อ่านกลับและตรวจทั้ง 6 ตัว: name, เวลาไทย/next run, status, cwd, prompt, execution, notification ให้ตรง แล้วสรุปสั้น ๆ ว่าตัวใดตั้งสำเร็จหรือล้มเหลว
...
stdout เป็น JSONL 6 บรรทัด; exit 0=ทุกตัวสด, 10=มีงานขาด/ค้าง, 1=มี error (สำคัญกว่า stale)
