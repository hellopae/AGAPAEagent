...
**Dale จับได้ว่า quality gate ไม่ได้บังคับอะไรจริงมาประมาณ 6 สัปดาห์** — หลักฐาน: `.gate-state.json`
ค้าง 29 ก.ค. · `.skill-state.json` ไม่เคยเกิดขึ้นเลยทั้งที่ hook เข้า `settings.json` ตั้งแต่ 27 ส.ค.
→ ที่ `CLAUDE.md` เขียนว่า gate "บังคับด้วยโค้ด ไม่ใช่ honor system" **ไม่เป็นความจริงในช่วงนั้น**

**✅ แต่สาเหตุที่เดากันไว้ตอนแรกผิด — ทดสอบแล้ววันเดียวกัน (ดู `BACKLOG.md` ข้อแรก)**
สมมุติฐานเดิมคือ "hook ตัวที่ 2/3 ของ block ไม่ยิงเพราะแย่ง stdin กัน" — **ไม่จริง**
ใส่ตัวบันทึก `scripts/hook-log.mjs` แล้วรัน `claude -p` จากโฟลเดอร์โปรเจกต์: **hook ยิงครบทุกตัว
อ่าน stdin ได้ครบเท่ากัน และ state ทั้งสองไฟล์ถูกเขียนใหม่จริง** → **ไม่มีบั๊ก ไม่มีอะไรต้องซ่อมในโค้ด**

สาเหตุจริง: บนแมคเครื่องนี้ไม่มี session ที่เปิดจากโฟลเดอร์โปรเจกต์เลยตั้งแต่ 29 ก.ค. (transcript 2 ไฟล์
เทียบกับ session ที่เปิดจาก home 22 ไฟล์ในเดือน ก.ย.) — **เปิดจาก home = ไม่โหลด `.claude/` = ไม่มี hook เลย**
คือ P0 เดิมที่ปิดไป 1 ส.ค. ไม่ใช่บั๊กใหม่ · แก้แล้วด้วยฟังก์ชัน `agapae` ใน `~/.zshrc`

> **บทเรียนที่ควรจำมากกว่าตัวบั๊ก:** หลักฐาน (mtime ค้าง) ถูก แต่การอธิบายสาเหตุผิด —
> ถ้าลงมือรื้อ `hook-chain.mjs` ตามข้อเสนอเดิมทันทีจะเสียเวลา 3 ชม. กับสิ่งที่ไม่ได้พัง
> **วัดก่อนซ่อมเสมอ:** `node scripts/hook-log-report.mjs`
